              ...
              <form action=" <?php echo base_url(); ?>dcadmin/Open_services/add_open_service_data/<? echo base64_encode(2); ?>/<?= $id; ?>" method="POST" id="slide_frm" enctype="multipart/form-data">
                <div class="table-responsive">
                  <table class="table table-hover">
                    ...
                    <tr>
                      <td> <strong>Service Date</strong> <span style="color:red;">*</span></strong> </td>
                      <td> <input type="date" name="service_date" class="form-control" placeholder="" value="<?= $services_data->service_date ?>" required /> </td>
                    </tr>
                    <tr>
...
<script>
  ...
  $(".plus").click(function() {
    ...
    var c_up = bla + 1;
    $('#count').val(c_up);
    $("#main").append('<div style="margin-top:5px" id="div_' + c_up + '"><select class="form-control" data-live-search="true" name="part_id_' + c_up +
      '" required><? foreach ($products_data->result() as $product) { ?><option value="<?= $product->id; ?>"><?= $product->name ?></option><? } ?></select><span style="display:flex"><input type="text" class="form-control" placeholder="Quantity" name="part_qty_' +
      c_up + '" style="width:60%" onkeypress="return isNumberKey(event)"><button style="margin-left:3px" onclick="remove_it(' + c_up + ')">-</button></span></div>');
    $('select').selectpicker();
